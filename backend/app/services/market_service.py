...
def get_crypto_market_data(page: int = 1, limit: int = 6) -> Dict:
    all_tickers = [
    ...
    start = (page - 1) * limit
    chunk = all_tickers[start:start + limit]

    cache_key = f"crypto:market_data:p{page}"
    cached_data = cache_get(cache_key)
    if cached_data:
        return cached_data

    logger.info(f"Fetching cryptos página {page}: {chunk}")
    with ThreadPoolExecutor(max_workers=limit) as executor:
        results = list(executor.map(get_crypto_data, chunk))

    cryptos = [r for r in results if r is not None]
    if len(cryptos) == 0:
        return {'stats': None, 'cryptos': [], 'page': page, 'total': len(all_tickers), 'limit': limit}
    ...
    if page == 1:
        ...
        stats = {'totalMarketCap': total_market_cap, 'totalVolume': total_volume, 'btcDominance': btc_dominance, 'fearGreed': 50}

    data = {'stats': stats, 'cryptos': cryptos, 'page': page, 'total': len(all_tickers), 'limit': limit}
...
def get_usa_stocks(page: int = 1, limit: int = 10) -> dict:
    start = (page - 1) * limit
    chunk = SP500_TICKERS[start:start + limit]
    cache_key = f"mercado:usa:p{page}"
    cached = cache_get(cache_key)
    if cached:
        return cached
    with ThreadPoolExecutor(max_workers=limit) as executor:
        results = list(executor.map(get_stock_data, chunk))
    stocks = [r for r in results if r is not None]
    result = {"data": stocks, "page": page, "total": len(SP500_TICKERS), "limit": limit}
...
def get_cedears(page: int = 1, limit: int = 10) -> dict:
    tickers = list(CEDEARS_DATA.keys())
    start = (page - 1) * limit
    chunk = tickers[start:start + limit]
    cache_key = f"mercado:cedears:p{page}"
    cached = cache_get(cache_key)
    if cached:
        return cached
    dolar_ccl = 1000
    ...
    result = {"data": cedears, "page": page, "total": len(tickers), "limit": limit}
    cache_set(cache_key, result, expire=300)
    return result
