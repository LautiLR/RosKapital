...
async def get_cryptos(request: Request, page: int = 1, limit: int = 6):
    return get_crypto_market_data(page=page, limit=limit)
...
@limiter.limit("60/hour")
async def get_stocks_usa(request: Request, page: int = 1, limit: int = 10):
    return get_usa_stocks(page=page, limit=limit)
...
@limiter.limit("60/hour")
async def get_stocks_cedears(request: Request, page: int = 1, limit: int = 10):
    return get_cedears(page=page, limit=limit)
...
async def get_economic_calendar(request: Request):
    ...
    try:
        calendar_file = Path(__file__).resolve().parent.parent.parent / "economic_events.json"
...
async def get_dolar_watch(request: Request):
    ...
    cache_file = Path("dolar_cache.json")
    ...
    try:
        async with httpx.AsyncClient() as client:
            tasks = [
                client.get(f"https://dolarapi.com/v1/dolares/{t['slug']}", timeout=5)
                for t in tipos
            ]
            responses = await asyncio.gather(*tasks, return_exceptions=True)
            for tipo, res in zip(tipos, responses):
                try:
